...
/// </summary>
[AddComponentMenu("UI/Input Field Ex")]
    ...
    ILayoutElement
{
    ...
    {
        // 不处理代理对 emoji
        if (EmojiParser.IsEmojiSingleChar(addedChar))
        {
            return ' ';
        }
        return addedChar;
    ...
    private bool CheckEmojiSupport()
    {
        return m_isSupportEmoji && m_emojiMap != null;
    }
    ...
    public OnFocusEvent _mOnFocusChanged = new OnFocusEvent();
    ...
    public string text
    {
        ...
        set
        {
            ...
            // Debug.LogWarningFormat("Input field's current text value. '{0}' '{1}'", m_Text, value);
        }
    }
    ...
    public OnFocusEvent onFocusChanged { get { return _mOnFocusChanged; } set { SetPropertyUtility.SetClass(ref _mOnFocusChanged, value); } }
        ...
        if (m_Keyboard == null || m_Keyboard.done)
        {
            if (m_Keyboard != null)
            {
                if (!m_ReadOnly)
                {
                    ...
                    {
                        text = m_emojiMap.Translate(m_Keyboard.text);
                    }
                    else
                    {
                    ...
                    // Debugger.LogWarning("Emoji  m_Keyboard.done m_Keyboard, text = '{0}' {1}", m_Keyboard.text, text);
                }
    ...
    private void SetKeyboardText(string str)
    {
        if (CheckEmojiSupport())
        {
            m_Keyboard.text = m_KeyboardText;
        }
        else
        {
            m_KeyboardText = str;
        ...
        // Debugger.LogWarning("Emoji  SetKeyboardText '{0}'", m_Keyboard.text);
    }
    ...
    public virtual void OnUpdateSelected(BaseEventData eventData)
    {
        ...
        while (Event.PopEvent(m_ProcessingEvent))
        {
            ...
            {
                if(m_ProcessingEvent.keyCode==KeyCode.None)
                {
                    strJoined += m_ProcessingEvent.character;
                    consumedEvent = false;
                }
                else
                {
    ...
    private string GetSelectedString()
    {
        ...
        var selectedString = text.Substring(startPos, endPos - startPos);
        if (CheckEmojiSupport())
        {
            selectedString = EmojiMap.InvTranslate(selectedString);
        }
    ...
    protected virtual void Append(string input)
    {
        ...
        if(CheckEmojiSupport())
        {
            input = m_emojiMap.Translate(input);
        }
    ...
    protected void UpdateLabel()
    {
        if (m_TextComponent != null && m_TextComponent.font != null && !m_PreventFontCallback)
        {
            ...
            string strTranslatedTxt = processed;
            ...
            if (!isEmpty)
            {
                ...
                if(CheckEmojiSupport())
                {
                    ...
                    {
                        processed = EmojiMap.ReplaceEmojiToSpace(processed);
                    }
                }
            ...
            if(CheckEmojiSupport())
            {
                TextEx soarDText = m_TextComponent as TextEx;
                if(soarDText != null)
                {
                    if(strTranslatedTxt != processed)
                    {
                        strTranslatedTxt = strTranslatedTxt.Substring(m_DrawStart, processed.Length);
                    }

                    // Debugger.LogWarning("Emoji  UpdateLabel end '{0}' ----- '{1}'", strTranslatedTxt, processed);
                    soarDText.SetEmojiText(strTranslatedTxt, processed);
                }
                else
                {
                    m_TextComponent.text = processed;
            ...
            // Debugger.LogWarning("Emoji  UpdateLabel m_TextComponent '{0}' end", m_TextComponent.text);
            MarkGeometryAsDirty();
    ...
    private void ActivateInputFieldInternal()
    {
        ...
        if (CheckEmojiSupport())
        {
            m_KeyboardText = EmojiMap.InvTranslate(m_Text);
        }
        else
        {
            m_KeyboardText = m_Text;
        }
        // Debugger.LogWarning("Emoji   TouchScreenKeyboard.Open '{0}' {1}", m_Text, m_KeyboardText);
        if (TouchScreenKeyboard.isSupported)
        {
            ...
            m_Keyboard = (inputType == InputType.Password) ?
                TouchScreenKeyboard.Open(m_KeyboardText, keyboardType, false, multiLine, true) :
                TouchScreenKeyboard.Open(m_KeyboardText, keyboardType, inputType == InputType.AutoCorrect, multiLine);
        ...
        // Debug.LogWarningFormat("ActivateInputFieldInternal '{0}' '{1}'", m_KeyboardText, m_Text);
