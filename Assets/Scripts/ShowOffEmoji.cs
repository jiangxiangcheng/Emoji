...
public class ShowOffEmoji : MonoBehaviour {
    ...
    public void OnTextValueChange(Text text)
    {
        StartCoroutine(SetUITextThatHasEmoji(text, text.text));
    }
    ...
    public IEnumerator SetUITextThatHasEmoji(Text textToEdit, string inputString)
    {
        ...
        if (textToEdit != null)
        {
            ...
            UICharInfo[] uICharInfos = textGen.GetCharactersArray();
            ...
            for (int j = 0; j < emojiReplacements.Count; j++)
            {
                ...
                Vector2 imagePos = uICharInfos[emojiIndex].cursorPos;
                imagePos.x = imagePos.x + uICharInfos[emojiIndex].charWidth / 2;
                imagePos.y = imagePos.y - uICharInfos[emojiIndex].charWidth / 2;
